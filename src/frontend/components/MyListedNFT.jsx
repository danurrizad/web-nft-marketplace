          ...
          (<div className='flex'>
            <div className='px-10 py-8 flex flex-wrap gap-10'>
              {items.map((item, index) => (
                <div key={index} className='rounded-xl bg-black border-2 border-b-0 border-black w-[150px] 2xl:w-[300px] xl:w-[300px] lg:w-[300px] min-w-[150px] h-fit '>
                  <div>
                    <h1 className='rounded-t-xl text-center 2xl:text-[20px] text-[15px] bg-slate-800 text-white p-1'>{item.name}</h1>
                    <img src={item.image} alt={"sold-items-"} />
                    <div className='rounded-b-xl shadow-xl shadow-slate-600 bg-slate-800 p-4 border-y-4 border-black flex flex-col text-center justify-center text-white 2xl:text-[23px] xl:text-[23px] lg:text-[23px] md:text-[20px] sm:text-[12px] text-[12px]'>
  ...
  if(isLoadingItems) return(
    <div className='bg-slate-300 min-h-screen'>
      <div id="myListedNFT" className=' w-full pb-10'>
        ...
        <div className='px-10 flex justify-start pb-32'>
        ...
        <div className='bg-slate-300 min-h-screen'>
            ...
            <div id="myListedNFT" className=' w-full pb-10'>
                ...
                {listedItems.length > 0 ? (
                    <div className='px-10 py-8 flex flex-wrap gap-10'>
                        {listedItems.map((item,index) => (
                            <div key={index} className='rounded-xl shadow-xl shadow-slate-600 bg-slate-800 border-solid border-2 border-b-0 border-black w-[150px] 2xl:w-[300px] xl:w-[300px] lg:w-[300px] min-w-[150px] h-fit '>
                                <h1 className='rounded-t-lg text-center 2xl:text-[20px] xl:text-[20px] lg:text-[20px] md:text-[15px] sm:text-[15px] text-[15px] bg-slate-800 text-white p-1'>{item.name}</h1>
                                <div className='p-0'><img src={item.image} alt={"item-nft-"+index}/></div>
                                <div className='rounded-b-lg bg-slate-800 p-4 border-y-4 border-black flex justify-center text-white 2xl:text-[23px] xl:text-[23px] lg:text-[23px] md:text-[12px] sm:text-[12px] text-[12px]'>
              ...
              <div className='flex border-0'>
                <h2 className='px-10 2xl:text-[30px] xl:text-[30px] lg:text-[30px] text-[20px]'>Want to create your own NFT and list it to the store? <button onClick={()=>setShowCreate(true)} className='2xl:text-[25px] font-bold text-slate-900 hover:text-slate-600 duration-300 hover:underline'>Create here</button></h2>
              </div>
